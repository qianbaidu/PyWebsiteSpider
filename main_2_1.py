...
class getHtml():
    def __init__(self):
        ...
        self.fileType = ['.png','.jpg','.jpeg','.bmp','.gif','.svg','.htc','.eot','.woff2','.ttf','.woff','.css']
        self.fileDir = ['css','images','js','swf','other']
        self.initFileArr()
        self.initDir()
    ...
    def test(self,url):
        ...
        if(len(html) > 0):
            ...
            # import css
            backImg=re.findall(r"\(.*?\)",html)
            for h in backImg:
                fUrl = h.replace('(','').replace(')','').replace('"','').replace("'",'')
                if fUrl.find('?') >= 0 :
                    strNum = fUrl.find('?')
                    fUrl = fUrl[0:strNum]

                fileUrl = os.path.split(fUrl)


                if len(fileUrl[1]) <= 0 :continue
                #print(fileUrl)
                imgType = os.path.splitext(fileUrl[1])
                #print(imgType)
                if len(imgType[1]) <= 0 : continue
                if imgType[1] not in self.fileType : continue
                fullFileUrl = urljoin(self.url,fUrl)
                print(fullFileUrl)
                if imgType[1] == '.css':
                    if fileUrl[1] in self.cssFile:
                        saveFileName = '%d_%d_%s' % (time.time(),random.randint(1000,9999),fileUrl[1])
                    else:
                        saveFileName =  fileUrl[1]
                    print(self.dir)
                    saveFilePath =  '%s/%s' % (self.dir['css'],saveFileName)
                    if len(saveFileName) > 0:
                        self.saveCss(fullFileUrl,saveFilePath)
                else:
                    if fileUrl[1] in self.imgFile:
                        saveFileName = '%d_%d_%s' % (time.time(),random.randint(1000,9999),fileUrl[1])
                    else:
                        saveFileName =  fileUrl[1]
                    saveFilePath =  '%s/%s' % (self.dir['images'],saveFileName)
                    print(saveFilePath)
                    if len(saveFileName) > 0:
                        fileContent = self.getByUrl(fullFileUrl)
                        if len(fileContent) > 0:
                            self.putOutFile(saveFilePath,fileContent)
